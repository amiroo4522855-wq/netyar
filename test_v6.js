...
for(const v of views){
  try{w.go(v);await sleep(30);if(bodyTxt().length>100)navOK++;else navBad.push(v);}
...
console.log('— تتریس —');
w.go('game-tetris');await sleep(30);
...
console.log('— مار —');
w.go('game-snake');await sleep(30);
...
console.log('— حافظه —');
w.go('game-memory');await sleep(30);
...
w.go('game-rps');await sleep(30);
$$('.rps-b')[0].click();
await sleep(700);
T('دور بازی شد',ev('RPS.me+RPS.pc+RPS.draw')===1);

console.log('— ری‌اکشن —');
w.go('game-react');await sleep(30);
$('.rct-panel').click();
...
console.log('— پازل —');
w.go('game-puzzle');await sleep(30);
...
console.log('— شطرنج —');
w.go('game-chess');await sleep(30);
...
w.go('home');await sleep(30);
T('نسخه ۶٫۰',bodyTxt().includes('نسخه ۶٫۰'));
w.go('sites');await sleep(80);
T('۵۱۲ سایت',$$('.content article.card').length===512);
w.go('home');await sleep(30);
